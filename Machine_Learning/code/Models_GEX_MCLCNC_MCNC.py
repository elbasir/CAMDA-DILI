...
pkl_file = open('gex_compound_identifier.pkl', 'rb')
...
#all
np.random.seed(66)
for train,test in skf.split(np.zeros(178),Y_all):
    train_list_all.append(train)
    test_list_all.append(test)
...
for file,file_ext in zip(files,files2):
    ...
    for dataset in range(2):
        ...
        if dataset == 0:
            ...
            #translate identifier in folds into np.array indices for current file
            train_list_curr = []
            test_list_curr = []
            for i,(train,test) in enumerate(zip(train_list_all,test_list_all)):
                train_list_curr.append([])
                test_list_curr.append([])
                for comp_tr in train:
                    #get all indices in that belong to unique compound
        ...
        for scr in range(4):
            ...
            for split,(train, test) in enumerate(zip(train_list_curr,test_list_curr)):
                ...
                #get RF parameters
                best_params[file+'.'+dict_dataset[dataset]+'RF.' + str(scr+1)+'.'+str(split+1)] = rf_param_selection(X[train_idx],Y[train_idx],
                            gkf)
...
df_ts.to_csv('ts_scores_GEX_yscr.csv',sep=',',index=False)
...
df_cv.to_csv('cv_scores_GEX_yscr.csv',sep=',',index=False)

#export best params
output = open('best_params_GEX_yscr.pkl','wb')
...
#export splits
output = open('train_list_all.pkl','wb')
pickle.dump(train_list_all, output)
output.close()

output = open('test_list_all.pkl','wb')
pickle.dump(test_list_all, output)
output.close()

output = open('train_list_mcnc.pkl','wb')
pickle.dump(train_list_mcnc, output)
output.close()

output = open('test_list_mcnc.pkl','wb')
